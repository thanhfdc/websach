<?php
	$query	= "SELECT `b`.`id`, `b`.`name`, `c`.`name` AS `category_name`, `b`.`picture`, `b`.`category_id` FROM `".TBL_BOOK."` AS `b`, `".TBL_CATEGORY."` AS `c` WHERE `b`.`status`  = 1 AND `b`.`special`  = 1 AND `b`.`category_id` = `c`.`id` ORDER BY `b`.`ordering` DESC LIMIT 0,2";
...
<div class="right_box">
	<!-- TITLE -->
	<?php echo Helper::createTitle("$imageURL/bullet4.gif", 'Đặc biệt');?>
